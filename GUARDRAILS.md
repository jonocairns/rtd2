...
- **Read-only investigation sub-agent** — `media_investigate` can run read-only tools for broad investigation and return grounded facts/proposed actions, but mutating actions still go through the main agent and confirmation gate.
...
**Implemented.** [`AuditLog`](src/audit.ts) writes JSONL to `logs/audit-<ts>.jsonl`. Events emitted: `session_start`, `tool_call` (from [repl.tsx](src/cli/repl.tsx) when the agent invokes a tool), `confirm_decision` (from [confirm.ts](src/confirm.ts) with the resolved human-readable lines and approve/decline outcome).
